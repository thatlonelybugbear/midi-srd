export async function glovesMissileSnaring(args) {
    if (!args[0].macroPass == "isAttacked") {
        const { options:{token:targetToken,actor:targetActor}, item } = args[0] ?? {};
        if (!token || !actor || !item) return ui.notifications.error("Gloves of Missile Snaring MidiSRD macro error, pleasy notify the GM");
    ...
    if (!args[0]) return;
    const {actor:deflectActor,token:deflectToken,itemCardId,workflowOptions:{sourceActorUuid,sourceItemUuid,sourceAmmoUuid,damageTotal}} = args[0] ?? {};
    if (!deflectActor || !deflectToken || !itemCardId || !sourceActorUuid || !sourceItemUuid || !damageTotal) return ui.notifications.error("Gloves of Missile Snaring MidiSRD macro error - Notify GM");
