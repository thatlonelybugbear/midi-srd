...
export async function magicMissile(args) {
	...
	if (args[0].targets.length === 1){
		...
		await MidiSRDHelpers._wait(1000);
	...
	if (args[0].targets.length > 1){
		let targetList = "";
		const all_targets = args[0].targets;
		...
		new Dialog({
			...
			one: { label: "Damage", callback: async (html) => {
                ...
                await game.dice3d?.showForRoll(damageRoll,game.user,true);
				for(const selected_target of selected_targets){
					const damageNum = selected_target.value;
					if (damageNum != null){
